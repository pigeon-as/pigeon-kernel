...
build() {
  ...
  make -C "build/linux-${VERSION}" O=build ARCH="$(arch)" CROSS_COMPILE="$(cross_compile)" olddefconfig
  make -C "build/linux-${VERSION}" O=build ARCH="$(arch)" CROSS_COMPILE="$(cross_compile)" -j"$(nproc)" "$(basename "$(image)")"
}
